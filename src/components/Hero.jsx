   ...
   <section id="home" className="w-full h-screen relative">
        ...
        <div className="relative w-full h-full">
           {slides.map((slide, idx) => (
            ...
            >
              <img src={slide.bgImg} alt={slide.title} className="w-full h-full object-cover p-1 rounded-3xl " />
              <div className="absolute inset-0 bg-gradient-to-t from-slate-950/80 via-slate-950/40 to-transparent" />
              <div className="absolute bottom-5 left-0 right-0 p-4 sm:p-6 md:p-8 text-white">
                <motion.h1
          ...
          <a href="#booking" className="absolute bottom-2 sm:bottom-2 left-4 sm:left-6 md:left-8 inline-flex items-center gap-2 sm:gap-3 px-3 sm:px-4 py-2 sm:py-3 bg-gradient-to-r from-[#FFD966] via-[#F2C94C] to-[#D4AF37] text-slate-950 font-semibold rounded-full uppercase tracking-[0.15em] sm:tracking-[0.22em] text-xs sm:text-sm shadow-[0_24px_60px_rgba(212,175,55,0.35)] transition-transform duration-300 hover:-translate-y-1 hover:shadow-[0_28px_72px_rgba(212,175,55,0.45)] z-20">
            Begin Your Journey
          </a>
           {/* Slide indicators */}
          <div className="absolute bottom-2 sm:bottom-2 left-1/2 transform -translate-x-1/2 flex gap-2 z-20">
            {slides.map((_, idx) => (
              <button
                key={idx}
